...
const Wrapper = styled.div<any>`
  position: relative;
  ${(props: any) => (props.embedded ? css`width: 100%;` : props.width ? css`width: ${props.width}px;` : '')}
`
...
class AutocompleteDropdown extends React.Component<Props, State> {
  ...
  getFilteredItems(props?: Props | null, searchValue?: string): any[] {
    ...
    const useSearch = searchValue === undefined ? this.state.searchValue : searchValue
    if (!useProps.items) {
      return []
    }
    return useProps.items.filter(i => {
      ...
      return label.indexOf(useSearch.toLowerCase()) > -1
  ...
  scrollIntoView(itemIndex?: number) {
    const selectedItemIndex = this.state.filteredItems
  ...
  updateListPosition() {
    if (!this.state.showDropdownList || !this.listRef || !this.firstItemRef
      || !this.buttonRef || !document.body || !this.buttonRect || !this.tipRef) {
      return
    }
  ...
  renderItems() {
    ...
    this.state.filteredItems.forEach((item, i) => {
      ...
      for (let j = i + 1; j < this.state.filteredItems.length; j += 1) {
        if (label === this.getLabel(this.state.filteredItems[j])
  ...
  renderList() {
    ...
    const firstItemValue = this.state.filteredItems.length > 0
  ...
  render() {
    const nullLabel = this.props.items && this.getValue(this.props.items[0]) === null ? this.getLabel(this.props.items[0]) : ''
    const inputValue = this.getValue(this.props.selectedItem) === null && this.state.searchValue === nullLabel ? '' : this.state.searchValue
